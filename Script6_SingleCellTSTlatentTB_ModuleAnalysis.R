## Script 6: Module analyses - Latent TB: Day 2 TST (single cell)
...
modules <- read.xlsx("../../../SupplementaryTables.xlsx",
  ...
  filter(!module %in% c("STAT1_Blood","TNF_Blood")) # exclude blood modules
...
write.csv(summary,"../../../data/ModuleCoverage_SingleCell_TSTD2_LatentTB.csv",row.names = F)

# extract module scores and cluster label
scores <- colData(sce) %>%
  as.data.frame() %>%
  dplyr::select("Sample","Sex","CellType",paste0(names(module_genes),"_module.score"))

# convert from wide to long
scores_long <- scores %>% 
  rownames_to_column("cell") %>%
  pivot_longer(cols = -c(cell,Sample,Sex,CellType), names_to = "module", values_to = "score")

# calculate Z scores per cell
zscores <- scores_long %>% group_by(module) %>% mutate(zscore = scale(score)) %>% ungroup()
# average by group
avg <- zscores %>% group_by(module,CellType,Sample,Sex) %>% summarise(Zscore_average = mean(zscore)) %>% ungroup()
...
addWorksheet(wb,"Fig3A,S3")
writeData(wb, "Fig3A,S3", avg)
